...
//  2019/11/14     meisq         1.1         add delay
//*******************************************************************************/
...
localparam MS_MAX_CNT = CLOCK_FREQ / 1000 - 1;//1ms 
wire scl_pad_i;
wire scl_pad_o;
wire scl_padoen_o;
...
begin
	...
	begin
		...
		case(state)
			...
			S_WR_I2C_CHECK:
			begin
				if(i2c_slave_dev_addr == 8'hfe)
				begin
					state <= S_WR_I2C_WAIT;
					
					ms_cnt <= 16'd1;
				end
				else if(i2c_slave_dev_addr != 8'hff)
				begin
					i2c_write_req <= 1'b1;
					state <= S_WR_I2C;
				end
			...
			S_WR_I2C_WAIT:
			begin
				if(timer_cnt >= MS_MAX_CNT)
				begin
					timer_cnt <= 32'd0; 
					if(ms_cnt == lut_reg_addr)
					begin
						lut_index <= lut_index + 10'd1;
						state <= S_WR_I2C_CHECK;
					end
